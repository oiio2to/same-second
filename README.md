...
`tools/make-cards.py` 会把任意一份符合结构的教程页转成卡片。做法：

- **页框固定** 430×573.34，统一 `scale(2.5116)` 放大到 1080×1440。每张卡的边框、虚线内框、四角位置逐像素一致——不会出现某几张露边线的情况。
- **只压内容不压框**。内容溢出的页，把内层容器加宽到 `100/k %` 再整体 `scale(k)`，视觉宽度仍撑满页框，效果等同于「字略小一点」而不是「整页缩小留白」。实际缩放比记在页框的 `data-k` 上。
- **预览自适应**。卡片是 1080px 宽，手机屏幕装不下；列表按屏宽整体缩放显示，导出前还原成 1:1，导出的仍是足尺 1080×1440。

一个容易踩的坑：flex 容器里元素比容器宽时，`justify-content:center` 会把左边推到负坐标，而浏览器不允许往负方向滚——左边不是没渲染，是滚不过去。所以预览必须缩放，不能靠滚动。
